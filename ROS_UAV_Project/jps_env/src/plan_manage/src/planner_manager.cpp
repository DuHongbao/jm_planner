...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  bool use_geometric_path, use_kinodynamic_path, use_topo_path, use_optimization, use_active_perception,use_jmjps_path;
  ...
  nh.param("manager/use_jmjps",use_jmjps_path,true);

  odom_sub_ = nh.subscribe("/odom_world",1,&FastPlannerManager::rcvOdometryCallbck, this);
  env_pub_ = nh.advertise<modu_msgs::modu>("env_msg",100);
  ...
  if(use_jmjps_path){
    jm_jps_.reset(new JmJps);
    jm_jps_->setEnvironment(edt_environment_);
    jm_jps_->init(nh);
  }

}

void FastPlannerManager::rcvOdometryCallbck(const nav_msgs::Odometry& odom) {
    //cout<<"FastPlannerManager recieved the odometry!"<<endl;
    Eigen::Vector3d pos (odom.pose.pose.position.x,odom.pose.pose.position.y,odom.pose.pose.position.z);
    double dist;
    Eigen::Vector3d dist_grad;
    edt_environment_->evaluateEDTWithGrad(pos, -1.0, dist, dist_grad);
    if (dist_grad.norm() > 1e-4) dist_grad.normalize();

    modu_msg.position.x = pos(0);
    modu_msg.position.y = pos(1);
    modu_msg.position.z = pos(2);
    modu_msg.direction.x = dist_grad(0);
    modu_msg.direction.y = dist_grad(1);
    modu_msg.direction.z = dist_grad(2);
    modu_msg.distance = dist;
    env_pub_.publish(modu_msg);
    //cout<<"position in "<<modu_msg.position<<" with "<<modu_msg.distance<<" and: "<<modu_msg.direction<<endl;
}
...
void FastPlannerManager::findCollisionRange(vector<Eigen::Vector3d>& colli_start,
  ...
  for (double tc = t_m; tc <= t_mp + 1e-4; tc += 0.05) {
    ...
    safe = edt_environment_->evaluateCoarseEDT(ptc, -1.0) < jm_jps_->clearance_ ? false : true;
...
//!SECTION  jmJps  replanning

bool FastPlannerManager::jmJpsReplan(bool collide){
    cout<<"运行到这里啦3！"<<endl;
    //如果轨迹发生碰撞，扩大搜索角点范围
    ...
    if(colli_start.size()==1&& colli_end.size() ==0 )
    {
      ...
      NonUniformBspline best_traj;
      ROS_INFO("JM_Planner--");
      vector<Eigen::Vector3d> j_path,j_pathT;
      cout<<"运行到这里啦7！"<<endl;
      cout<<"colli_start:"<<colli_start.size()<<endl;
      cout<<"colli_end:"<<colli_end.size()<<endl;
      cout<<"start_pts:"<<start_pts.size()<<endl;
      cout<<"end_pts:"<<end_pts.size()<<endl;
      jm_jps_->findJpsPath(start_pts.front(),end_pts.back(),  j_path);
      //Eigen::Vector3d end_state(6);
      if(j_path.size()==0){
      ROS_WARN("No path!");
      return false;
      }
      Eigen::Vector3d v1=j_path[1]-j_path[0];
      Eigen::Vector3d v2=j_path[j_path.size()-1]-j_path[0];
      if(v1.dot(v2) < 0.0){
        cout<<"path is not optimal!"<<endl;
            //jm_jps_->findJpsPath(start_pts.front(),end_pts.back(),  j_pathT);

        return false;
      }
      plan_data_.addJmJpsPaths(j_path);

      ROS_INFO("[Optimize]: ---------");

      plan_data_.jmjps_path_.resize(j_path.size());
      optimizeJmBspline(t_now, local_traj_duration, j_path);
      double t_opt = (ros::Time::now() - t1).toSec();
      cout << "[planner]: optimization time: " << t_opt << endl;


      best_traj = plan_data_.jm_traj_pos2_;
      refineTraj(best_traj, time_inc);
      local_data_.position_traj_ = best_traj;
      global_data_.setLocalTraj(local_data_.position_traj_, t_now,
                              local_traj_duration + time_inc + t_now, time_inc);
...
void FastPlannerManager::optimizeJmBspline(double start_t, double duration, vector<Eigen::Vector3d> path){
  ros::Time  t1;
  double  tm1,tm2,tm3;

  t1 = ros::Time::now();
  int seg_num = jm_jps_->pathLength(path)/pp_.ctrl_pt_dist;
  Eigen::MatrixXd ctrl_pts;
  double dt;
  ...
  vector<Eigen::Vector3d> guide_pt;
  guide_pt = jm_jps_->pathToGuidePts(path, int(ctrl_pts.rows()) - 2);
  ...
  bspline_optimizers_[0]->setGuidePath(guide_pt);  //only one path need optimize
  Eigen::MatrixXd opt_ctrl_pts1 = bspline_optimizers_[0]->BsplineOptimizeTraj(
      ctrl_pts, dt, BsplineOptimizer::GUIDE_PHASE, 0, 1);

  plan_data_.jm_traj_pos1_ = NonUniformBspline(opt_ctrl_pts1, 3, dt);
  ...
  plan_data_.jm_traj_pos2_ = NonUniformBspline(opt_ctrl_pts2, 3, dt);

  tm3 = (ros::Time::now() - t1).toSec();
  ROS_INFO("optimization 0 cost %lf, %lf, %lf seconds.",  tm1, tm2, tm3);
