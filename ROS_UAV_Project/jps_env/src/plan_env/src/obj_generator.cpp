...
int main(int argc, char** argv) {
  ...
  node.param("obj_generator/vel", _vel, 1.0);
  node.param("obj_generator/yaw_dot", _yaw_dot, 1.0);
  ...
  for (int i = 0; i < obj_num; ++i) {
    ...
    Eigen::Vector3d pos(rand_pos(eng), 0.2*rand_pos(eng), rand_h(eng));
    ...
    model.setLimits(Eigen::Vector3d(_xy_size, 0.2*_xy_size, _h_size), Eigen::Vector2d(0.0, _vel),
                    Eigen::Vector2d(0, 0));
...
void visualizeObj(int id) {
  ...
  mk.color.a = 1, mk.color.r = color(0), mk.color.g = color(1), mk.color.b = color(2);
