...
private:
  ...
  void dynamicEvent_callback(const std_msgs::Bool  dyEvent);

  SO3Control      controller_;
  ros::Publisher  so3_command_pub_;
  ros::Subscriber odom_sub_;
  ros::Subscriber position_cmd_sub_;
  ros::Subscriber dynamic_event_sub_;
  ros::Subscriber ctrl_modu_sub_;
...
void
SO3ControlNodelet::modu_cmd_callback(
  const quadrotor_msgs::PositionCommand::ConstPtr& cmd)
{
  des_pos_ = Eigen::Vector3d(cmd->position.x, cmd->position.y, cmd->position.z);
  des_vel_ = Eigen::Vector3d(cmd->velocity.x, cmd->velocity.y, cmd->velocity.z);
...
void SO3ControlNodelet::dynamicEvent_callback(const std_msgs::Bool dyEvent){

    SO3ControlNodelet::Is_event = dyEvent.data;
    cout<<"Is_event："<<Is_event<<endl;
}
...
SO3ControlNodelet::onInit(void)
{
  ...
  dynamic_event_sub_ =  n.subscribe("/Is_there_dynObs",10,&SO3ControlNodelet::dynamicEvent_callback,this, ros::TransportHints().tcpNoDelay() );
  
  ctrl_modu_sub_ = n.subscribe("/modu_cmd", 10, &SO3ControlNodelet::modu_cmd_callback, this, ros::TransportHints().tcpNoDelay());

  position_cmd_sub_ =  n.subscribe("position_cmd", 10, &SO3ControlNodelet::position_cmd_callback, this, ros::TransportHints().tcpNoDelay());
