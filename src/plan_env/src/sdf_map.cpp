...
void SDFMap::raycastProcess() {
  ...
  cout<<"this is raycast1"<<endl;
  if (md_.proj_points_cnt == 0) return;
  cout<<"this is raycast2"<<endl;
  ros::Time t1, t2;
  ...
  cout<<"map_max_boundary"<<mp_.map_max_boundary_<<endl;
  cout<<"map_min_boundary"<<mp_.map_min_boundary_<<endl;
  RayCaster raycaster;
  ...
  cout<<"local_bound_max and local_bound_min is"<<md_.local_bound_max_<<"  "<<md_.local_bound_min_<<endl;
...
void SDFMap::updateOccupancyCallback(const ros::TimerEvent& /*event*/) {
  ...
  cout<<"this is updateOccupancyCallback"<<endl;
  /* update occupancy */
  ros::Time t1, t2;
  t1 = ros::Time::now();
...
void SDFMap::updateESDFCallback(const ros::TimerEvent& /*event*/) {
  ...
  //cout<<"this is updateESDFCallback"<<endl;
  /* esdf */
  ros::Time t1, t2;
  t1 = ros::Time::now();
  ...
  //cout<<"ESDF: cur t = "<<(t2 - t1).toSec()<<endl;
  md_.esdf_need_update_ = false;
}
...
void SDFMap::depthPoseCallback(const sensor_msgs::ImageConstPtr& img,
  ...
  cout<<"This is depth Pose Callback"<<endl;
  /* get depth image */
...
void SDFMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& img) {
  //cout<<"This is PointCloud2 Callback"<<endl;
